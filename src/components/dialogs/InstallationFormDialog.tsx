...
import type { Installation, InstallationStatus, InstallationType } from "@/data/types";
...
export function InstallationFormDialog({ open, onOpenChange, installation }: Props) {
  const { addInstallation, updateInstallation, addInstallationMaterial, upsertFinancial, installationTemplates, templateMaterials } = useAppStore();
  ...
  useEffect(() => {
    if (open) {
      setForm(installation ? { ...installation } : { ...empty });
      setSelectedTemplate("");
      setErrors({});
    }
  }, [open, installation]);
  ...
  const save = async () => {
    ...
    try {
      if (installation) {
        await updateInstallation(installation.id, form);
      } else {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>{installation ? "Edit Installation" : "New Installation"}</DialogTitle>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            {errors.power_kva && <p className="text-xs text-destructive mt-1">{errors.power_kva}</p>}
          </div>
          <div>
            ...
            {errors.battery_kwh && <p className="text-xs text-destructive mt-1">{errors.battery_kwh}</p>}
          </div>
          <div>
          ...
          <div className="col-span-2 border-t pt-3 mt-1">
            <p className="text-xs font-medium text-muted-foreground mb-2">Voertuig & Keuring</p>
          </div>
          <div>
          ...
          <div>
            ...
            <Input type="date" value={form.volledig_nazicht_date ?? ""} onChange={(e) => set("volledig_nazicht_date", e.target.value || null as any)} />
          </div>
          <div>
            <label className="text-sm text-muted-foreground">Vervaldag autokeuring</label>
            <Input type="date" value={form.vervaldag_autokeuring ?? ""} onChange={(e) => set("vervaldag_autokeuring", e.target.value || null as any)} />
          </div>
          <div>
            <label className="text-sm text-muted-foreground">Nieuwe keuring ingepland</label>
            <Input type="date" value={form.nieuwe_keuring_date ?? ""} onChange={(e) => set("nieuwe_keuring_date", e.target.value || null as any)} />
          </div>
          <div className="flex items-center gap-4 col-span-2">
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={form.tracing_placed} onChange={(e) => set("tracing_placed", e.target.checked as any)} className="rounded" />
              Tracing geplaatst
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={form.verhuurklaar} onChange={(e) => set("verhuurklaar", e.target.checked as any)} className="rounded" />
              Verhuurklaar
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={form.map_in_orde} onChange={(e) => set("map_in_orde", e.target.checked as any)} className="rounded" />
              Map in orde
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={form.arei_schema_in_sg} onChange={(e) => set("arei_schema_in_sg", e.target.checked as any)} className="rounded" />
              AREI schema in SG
            </label>
          </div>
          <div className="col-span-2 border-t pt-3 mt-1">
            <p className="text-xs font-medium text-muted-foreground mb-2">Verzekering</p>
          </div>
          <div>
          ...
          <div>
            ...
            <Input type="date" value={form.verzekering_vervaldatum ?? ""} onChange={(e) => set("verzekering_vervaldatum", e.target.value || null as any)} />
          </div>
          <div className="col-span-2">
        ...
        <DialogFooter>
          ...
          <Button onClick={save} disabled={saving}>
            ...
            {saving ? "Saving…" : installation ? "Save Changes" : "Create"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
